...
export const SpawnGameMapSystem = (): System<{
  ...
  run: async ({ essence }) => {
    const entities = Essence.getEntitiesByComponents(essence, [SpawnGameMapEventComponentName]);
    ...
    const playerCameraEntityId = Essence.getEntitiesByComponents(essence, [
      PlayerComponentName,
      CameraComponentName,
    ])[0];
